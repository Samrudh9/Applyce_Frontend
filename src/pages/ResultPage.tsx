...
import {
  ...
  Eye, EyeOff, ExternalLink, FileText, GraduationCap, Lightbulb, Map, RotateCcw, Search,
      ...
      <div className="mx-auto max-w-5xl space-y-8">
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          ...
          <Card hover={false} className="flex flex-col items-center justify-center text-center">
            <h3 className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-ink-sec">Overall Score</h3>
            <CircularProgress value={overallScore} size={140} strokeWidth={10} label="" />
            <p className="mt-1 text-2xl font-bold text-ink">{overallScore}%</p>
              ...
              <div className="mt-4 w-full border-t border-line pt-4">
                <div className="flex items-center gap-2.5">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-accent-soft">
                    <FileText size={15} className="text-accent-strong" />
                  </div>
          ...
          {/* Score Breakdown */}
          <Card hover={false}>
            <h3 className="mb-4 text-[11px] font-semibold uppercase tracking-wider text-ink-sec">Score Breakdown</h3>
            <div className="space-y-4">
              <ProgressBar label="Keywords" value={keywordScore} showValue colorClass="from-accent-strong to-accent" />
              <ProgressBar label="Format" value={formatScore} showValue colorClass="from-accent-strong/80 to-accent/50" />
              <ProgressBar label="Sections" value={sectionScore} showValue colorClass="from-warning/80 to-warning/50" />
              <ProgressBar label="Content" value={contentScore} showValue colorClass="from-success/80 to-success/50" />
            </div>
          </Card>

          {/* Grade */}
          <Card hover={false} className="flex flex-col justify-center">
            <h3 className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-ink-sec">Grade</h3>
          ...
          <Card hover={false}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-ink flex items-center gap-2">
                <FileText size={16} className="text-accent-strong" /> Resume Preview
              </h3>
              <button
        ...
        <section>
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-bold text-ink">Career Matches</h2>
            <Badge tone="neutral" size="sm">Top matches</Badge>
          </div>
          <div className="grid gap-3 md:grid-cols-3">
            {result.predictions.slice(0, 3).map((career, i) => {
              const Icon = icons[i] ?? Target;
              return (
                <Card key={career.career} hover>
                  <div className="mb-3 flex items-center gap-2">
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-accent/10"><Icon size={18} className="text-accent-strong" /></div>
                  </div>
                  <h3 className="text-base font-bold text-ink">{career.career}</h3>
                  <p className="mt-1 text-sm text-ink-sec">Confidence {career.confidence.toFixed(1)}%</p>
                  <ProgressBar value={career.confidence} className="mt-2" colorClass="from-accent to-accent-strong" />
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          <Card hover={false}>
            <h2 className="mb-4 text-base font-bold text-ink">Your Skills <span className="text-sm font-normal text-ink-sec">({result.skills.length})</span></h2>
            <div className="flex flex-wrap gap-1.5">{result.skills.map((s) => <Badge key={s} tone="success" size="sm">{s}</Badge>)}</div>
          </Card>
          <Card hover={false}>
            ...
            {loadingGap && !gap ? (
              ...
              <div className="flex flex-wrap gap-1.5">
                {gap?.missing_skills?.length ? gap.missing_skills.map((s) => <Badge key={s} tone="warning" size="sm">{s}</Badge>) : <p className="text-sm text-ink-sec">Nothing missing for your top career match.</p>}
              </div>
            )}
          </Card>
        </section>
          ...
          return (
            <section>
              <div className="mb-4 flex items-center justify-between">
                ...
                <Badge tone="danger" size="sm">{(rf?.count ?? 0) + weaknesses.length} flagged</Badge>
              </div>
              ...
              {hasWeaknesses && (
                <div className="space-y-3">
                  {weaknesses.map((w, i) => (
                    <div key={i} className={`rounded-xl border border-line bg-surface p-5 ${w.severity === 'critical' ? 'border-danger/40' : w.severity === 'high' ? 'border-warning/40' : w.severity === 'medium' ? 'border-warning/30' : 'border-line-strong'}`}>
                      <div className="flex flex-wrap items-start justify-between gap-2">
                        <h3 className="font-semibold text-ink">{w.title}</h3>
                        <Badge tone={severityTone(w.severity)} size="sm">{w.severity}</Badge>
                      </div>
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          {/* ATS Report Preview */}
          <Card hover={false}>
            <h2 className="mb-4 text-base font-bold text-ink">ATS Report Preview</h2>
          ...
          {/* Recommendations */}
          <Card hover={false}>
            <h2 className="mb-4 text-base font-bold text-ink">Recommendations</h2>
            <div className="space-y-3">
              {improvements.length > 0 ? improvements.slice(0, 4).map((item, i) => (
                <div key={i} className="rounded-xl border border-line bg-elevated/60 p-4">
        ...
        {((result.education?.length ?? 0) > 0 || (result.experience?.length ?? 0) > 0 || (result.projects?.length ?? 0) > 0) && (
          <section className="grid gap-4 lg:grid-cols-3">
            {result.education && result.education.length > 0 && (
              <Card hover={false}>
                <h3 className="mb-3 text-base font-bold text-ink flex items-center gap-2"><GraduationCap size={18} className="text-accent-strong" /> Education</h3>
                <ul className="space-y-1">{result.education.map((e, i) => <li key={i} className="text-sm text-ink-sec">{e}</li>)}</ul>
              </Card>
            )}
            {result.experience && result.experience.length > 0 && (
              <Card hover={false}>
                <h3 className="mb-3 text-base font-bold text-ink flex items-center gap-2"><BookOpen size={18} className="text-accent-strong" /> Experience</h3>
                <ul className="space-y-1">{result.experience.map((e, i) => <li key={i} className="text-sm text-ink-sec">{e}</li>)}</ul>
              </Card>
            )}
            {result.projects && result.projects.length > 0 && (
              <Card hover={false}>
                <h3 className="mb-3 text-base font-bold text-ink flex items-center gap-2"><Star size={18} className="text-warning" /> Projects</h3>
        ...
        <Card hover={false}>
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <h2 className="text-lg font-bold text-ink">Take action</h2>
              <p className="mt-1 text-sm text-ink-sec">Own the result — download the report, fix the resume, or start applying.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button onClick={downloadReport} size="sm"><Download size={14} /> Download Report</Button>
              <Link to="/resume-builder" state={{ analysisResult: result }}>
                <Button variant="secondary" size="sm"><Wrench size={14} /> Fix in Resume Builder</Button>
              </Link>
              <Link to={`/jobs?career=${encodeURIComponent(result.predictions[0]?.career ?? '')}&skills=${result.skills.join(',')}`}>
                <Button variant="secondary" size="sm"><Search size={14} /> Search Jobs</Button>
              </Link>
              <Link to={`/roadmap?career=${encodeURIComponent(result.predictions[0]?.career ?? '')}`}>
                <Button variant="outline" size="sm"><Map size={14} /> View Roadmap</Button>
              </Link>
            </div>
          </div>
        </Card>
