        ...
        <div className="space-y-6 px-4 py-8 sm:px-6">
            <div className="mx-auto max-w-5xl">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <Button onClick={openNew} size="sm"><Plus size={14} /> New Resume</Button>
                </div>

                {/* Editor Modal */}
                {showEditor && (
                    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={() => setShowEditor(false)}>
                        <div onClick={(e) => e.stopPropagation()} className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border border-line bg-surface p-6 shadow-xl">
                            <div className="mb-6 flex items-center justify-between">
                                <h3 className="text-lg font-bold text-ink">{editingId ? 'Edit Resume' : 'New Resume'}</h3>
                                <button onClick={() => setShowEditor(false)} className="rounded-lg p-1 text-ink-ter hover:bg-elevated hover:text-ink"><X size={18} /></button>
                            </div>
                            <div className="space-y-5">
                                ...
                                <div>
                                    <h4 className="mb-3 text-sm font-semibold text-ink">Profile</h4>
                                ...
                                <div>
                                    <div className="mb-2 flex items-center justify-between"><h4 className="text-sm font-semibold text-ink">Experience</h4><button onClick={addExperience} className="text-xs font-medium text-accent-strong hover:text-accent">+ Add</button></div>
                                    {data.experience.map((exp, i) => (
                                        <div key={i} className="mb-3 rounded-lg border border-line p-3 space-y-2">
                                ...
                                <div>
                                    <div className="mb-2 flex items-center justify-between"><h4 className="text-sm font-semibold text-ink">Education</h4><button onClick={addEducation} className="text-xs font-medium text-accent-strong hover:text-accent">+ Add</button></div>
                                ...
                                <div>
                                    <div className="mb-2 flex items-center justify-between"><h4 className="text-sm font-semibold text-ink">Projects</h4><button onClick={addProject} className="text-xs font-medium text-accent-strong hover:text-accent">+ Add</button></div>
                                ...
                                <div>
                                    <h4 className="mb-3 text-sm font-semibold text-ink">Skills</h4>
                                    {([
                                        ...
                                        <div key={key} className="mb-3">
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button onClick={() => addSkills(key, input, setter)} className="shrink-0 rounded-lg bg-accent/10 px-3 text-xs font-medium text-accent-strong hover:bg-accent/20">Add</button>
                                            </div>
                                ...
                                <div className="flex justify-end gap-3 border-t border-line pt-4">
                                    <Button variant="outline" onClick={() => setShowEditor(false)} size="sm">Cancel</Button>
                                    <Button onClick={handleSave} disabled={saving} size="sm">
                                        {saving ? 'Saving…' : <><Save size={14} /> {editingId ? 'Update Resume' : 'Create Resume'}</>}
                                    </Button>
                                </div>
                            </div>
                        </div>
                ...
                {!loading && resumes.length === 0 && (
                    <Card hover={false} className="py-14 text-center">
                        <FileText size={40} className="mx-auto text-ink-sec" />
                        <p className="mt-3 text-base font-semibold text-ink">No resumes yet</p>
                        <p className="mt-1 text-sm text-ink-sec">Your first ATS-ready resume is minutes away.</p>
                        <Button onClick={openNew} className="mt-5" size="sm"><Plus size={14} /> Create Resume</Button>
                    </Card>
                )}
                ...
                {!loading && resumes.length > 0 && (
                    <div className="space-y-4">
                        ...
                        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                            {resumes.map((r) => (
                                <Card key={r.id}>
                                    <div className="mb-2 flex items-center gap-2"><FileText size={16} className="text-accent-strong" /><h3 className="text-sm font-semibold text-ink">{r.title}</h3></div>
