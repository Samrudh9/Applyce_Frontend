    ...
    <div className="space-y-8">
      ...
      <Card>
        ...
        <div className="space-y-4">
          <ProgressBar label="Keyword Match" value={keywordScore} showValue animated colorClass={keywordScore >= 80 ? 'from-accent-strong to-accent' : keywordScore >= 60 ? 'from-amber-400 to-amber-300' : 'from-red-400 to-red-300'} />
          <ProgressBar label="Format & Structure" value={formatScore} showValue animated colorClass={formatScore >= 80 ? 'from-accent-strong to-accent' : formatScore >= 60 ? 'from-amber-400 to-amber-300' : 'from-red-400 to-red-300'} />
          <ProgressBar label="Section Coverage" value={sectionScore} showValue animated colorClass={sectionScore >= 80 ? 'from-accent-strong to-accent' : sectionScore >= 60 ? 'from-amber-400 to-amber-300' : 'from-red-400 to-red-300'} />
        </div>
      </Card>
            ...
            <Card key={key}>
              ...
              <div className="space-y-2 text-sm">
                {Object.entries(obj).map(([subKey, subVal]) => (
                  <motion.div key={subKey} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between gap-4 rounded-xl border border-border bg-elevated/60 px-4 py-3">
      ...
      {qualityTips.length > 0 && qualityTips[0] !== 'Resume analysis completed' && (
        <Card>
          <h3 className="mb-3 flex items-center gap-2 text-lg font-bold text-ink"><Lightbulb size={18} className="text-amber-500" /> Improvement Tips</h3>
